...
import { Search, Building2, Zap, Globe, Shield, Users, MapPin, Verified, ArrowRight } from "lucide-react";
    ...
    <main style={{ minHeight: "100vh", background: "var(--bg-dark)", display: "flex", flexDirection: "column" }}>
      ...
      <section style={{ position: "relative", overflow: "hidden", padding: "100px 24px", textAlign: "center", flex: 1 }}>
        ...
        <div style={{ position: "relative", zIndex: 1, maxWidth: 860, margin: "0 auto" }} className="animate-fadeInUp">
          ...
          <form action="/search" method="GET" style={{ display: "flex", gap: 12, maxWidth: 640, margin: "0 auto 48px", flexWrap: "wrap", justifyContent: "center" }}>
            ...
            <button type="submit" className="btn-glow" style={{ padding: "14px 32px", borderRadius: 12, whiteSpace: "nowrap" }}>Search</button>
          </form>
      ...
      <section style={{ padding: "40px 24px", maxWidth: 1100, margin: "0 auto", width: "100%" }}>
        <div className="glass-card" style={{ padding: "40px 48px", background: "linear-gradient(135deg, rgba(108,63,255,0.12), rgba(255,107,53,0.08))", borderColor: "rgba(108,63,255,0.3)" }}>
          <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 32, justifyContent: "space-between" }}>
            <div style={{ flex: 1, minWidth: 260 }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "4px 14px", borderRadius: 99, marginBottom: 16, fontSize: 12, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", background: "rgba(108,63,255,0.2)", color: "#a78bfa" }}>
              ...
              <p style={{ color: "#888", fontSize: 15, lineHeight: 1.7 }}>
                Type any HSN code (e.g. <strong style={{ color: "#a78bfa" }}>6908</strong> for Ceramic Tiles or <strong style={{ color: "#a78bfa" }}>8484</strong> for Bearings) and instantly discover all verified dealers, manufacturers and distributors dealing in those products — anywhere in the world.
              </p>
            </div>
            <form action="/search" method="GET" style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ position: "relative" }}>
                <Zap style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", color: "#a78bfa", width: 16, height: 16 }} />
                <input
                  id="hsn-search"
                  name="q"
                  type="text"
              ...
              <button type="submit" className="btn-glow" style={{ padding: "12px 24px", borderRadius: 10, fontSize: 14 }}>Search HSN</button>
            </form>
          </div>
        </div>
      </section>
      ...
      {/* Featured Listings */}
      <section style={{ padding: "40px 24px 20px", maxWidth: 1100, margin: "0 auto", width: "100%" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 32, flexWrap: "wrap", gap: 12 }}>
          ...
          <Link href="/businesses">
            <button className="btn-glow" style={{ padding: "10px 22px", borderRadius: 10, fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
          {[
            { slug: "stark-industries",  name: "Stark Industries",   location: "Mumbai, Maharashtra", hsn: ["8484", "8544"], src: "https://images.unsplash.com/photo-1560179707-f14e90ef3623?auto=format&fit=crop&q=80&w=600" },
            { slug: "acme-corp",         name: "Acme Corp Ltd.",     location: "Ahmedabad, Gujarat",  hsn: ["6908", "6907"], src: "https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?auto=format&fit=crop&q=80&w=600" },
            { slug: "globe-logistics",   name: "Globe Logistics",   location: "New York, USA",        hsn: ["8708"],          src: "https://images.unsplash.com/photo-1464938050520-ef2270bb8ce8?auto=format&fit=crop&q=80&w=600" },
          ].map((b) => (
            <div key={b.slug} className="glass-card" style={{ overflow: "hidden", display: "flex", flexDirection: "column", cursor: "pointer" }}>
              <div style={{ height: 160, position: "relative", overflow: "hidden" }}>
                <img src={b.src} alt={b.name} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.5s ease" }} />
                <div style={{ position: "absolute", top: 10, left: 10 }}>
                  <span style={{ background: "rgba(16,185,129,0.9)", backdropFilter: "blur(4px)", color: "#fff", fontSize: 11, fontWeight: 700, padding: "3px 8px", borderRadius: 6, display: "flex", alignItems: "center", gap: 4 }}>
                    <Verified style={{ width: 11, height: 11 }} /> Verified
                  </span>
                </div>
              </div>
              <div style={{ padding: "16px 20px 20px", flex: 1, display: "flex", flexDirection: "column" }}>
                <h3 style={{ fontWeight: 800, fontSize: 17, color: "#fff", marginBottom: 4 }}>{b.name}</h3>
                <p style={{ color: "#888", fontSize: 13, display: "flex", alignItems: "center", gap: 4, marginBottom: 14 }}>
                  <MapPin style={{ width: 13, height: 13, color: "#ff6b35" }} /> {b.location}
                </p>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 16 }}>
                  {b.hsn.map(code => (
                    <span key={code} style={{ background: "rgba(108,63,255,0.12)", border: "1px solid rgba(108,63,255,0.25)", color: "#a78bfa", fontSize: 11, fontWeight: 600, padding: "3px 8px", borderRadius: 6 }}>HSN {code}</span>
                  ))}
                </div>
                <Link href={`/business/${b.slug}`} style={{ marginTop: "auto" }}>
                  <button style={{ width: "100%", background: "rgba(255,255,255,0.08)", border: "none", color: "#fff", borderRadius: 8, padding: "10px", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 6, transition: "background 0.2s" }}>
