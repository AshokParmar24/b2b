...
import { HsnForm } from "@/components/admin/masters/hsn/HsnForm";
import { Loader2, Tag, ArrowLeft } from "lucide-react";
import { Button } from "@/components/ui/button";
import { api } from "@/lib/api";

export default function HsnEditPage() {
  const { id } = useParams();
  ...
  const fetchHsn = async () => {
    try {
      const data = await api.get<any>(`/api/masters/hsn/${id}`);
      setHsn(data);
    } catch (error) {
    ...
    return (
      <div className="flex h-[70vh] w-full flex-col items-center justify-center p-6 text-center animate-in fade-in zoom-in duration-500">
        ...
        <div className="max-w-md space-y-3">
          <h2 className="text-3xl font-black tracking-tight text-foreground">HSN Record Missing</h2>
          <p className="text-muted-foreground font-bold text-base leading-relaxed">
            The system could not locate this specific HSN record. It may have been removed or the ID is incorrect.
          </p>
        </div>

        <Link href="/admin/masters/hsn" className="mt-10">
  ...
  return (
    <div className="py-8">
      <HsnForm initialData={hsn} isEdit />
    </div>
  );
}
